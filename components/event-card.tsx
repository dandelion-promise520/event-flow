...
export default function EventCard({ event }: EventProps) {
  ...
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-2xl border border-neutral-200 bg-white transition-all duration-300 hover:-translate-y-1 hover:border-neutral-300 hover:shadow-lg">
      <div className="aspect-video w-full bg-neutral-100 overflow-hidden relative">
        {event.coverUrl ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-neutral-400 font-semibold">
            {event.category}
          </div>
        )}
        <span className="absolute left-3 top-3 rounded-full bg-white/95 px-2.5 py-1 text-xs font-semibold text-neutral-800 shadow-sm">
          {event.category}
        </span>
      </div>
      <div className="flex flex-1 flex-col p-5">
        <h3 className="font-bold text-neutral-900 group-hover:text-indigo-600 line-clamp-1 transition-colors">
          {event.title}
        </h3>
        <p className="mt-2 text-xs text-neutral-500 line-clamp-2">{event.description}</p>
